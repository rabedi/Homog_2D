...
public:
	...
	void ComputeInverse(MATRIX& invMatrix);
...
inline void Product(MATRIX& ProductMat, const MATRIX& LeftMat, const MATRIX& RightMat)
{
	// Note...should have an implementation using cblas_dgemm
	int m = LeftMat.rows(), n = RightMat.columns(), p = LeftMat.columns();
	ProductMat.resize(m, n);
	ProductMat = 0.0;
	for (int i = 0; i < m; ++i)
		for (int j = 0; j < n; ++j)
			for (int k = 0; k < p; ++k)
				ProductMat[i][j] += LeftMat[i][k] * RightMat[k][j];
};
